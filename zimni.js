...
const ZIMNI_VER = 'zimni v18 — print numbering fixed across split bodies';
...
function _zimniPrintHTML(inner) {
  ...
  return `<!DOCTYPE html><html dir="rtl" lang="ur"><head><meta charset="UTF-8"><title> </title>
    ...
    </style></head><body><div id="ch173-doc">${inner}</div>
    <script>
      /* ═══ کالم 2 کے نمبروں کی سیدھ — چھپائی کی اپنی ناپ سے ═══
         اسکرین پر ناپا ہوا marginTop چھپائی میں بعض اوقات ٹھیک نہیں بیٹھتا
         (فونٹ دیر سے لوڈ ہونا، خانوں کی اونچائی وغیرہ)۔ اس لیے یہاں پرنٹ
         دستاویز کے اندر ہی، اُسی کی اصل ناپ سے، ہر نمبر کو اُس کے پیراگراف
         کے برابر کر دیا جاتا ہے۔ manual-space والے خانے نہیں چھیڑے جاتے۔ */
      (function(){
        ...
        function one(nums){
          ...
          // AHEM: Enter dabane par Chrome .zf-body ko kai bhaiyon me tor deta
          // hai — sirf pehla body padhne se aage wale paragraph chhoot jate the
          // aur PRINT me number aage-peeche ho jate the (screen theek tha kyunke
          // _zimniParas saare bodies dekhta hai). Ab yahan bhi SAARE bodies.
          ...
          for(i=0;i<kids.length && i<tops.length;i++){
            ...
            if(g>0&&g<40000) kids[i].style.marginTop=g+'px';   // کئی صفحوں کا فاصلہ بھی
          }
        }
        function run(){ [].slice.call(document.querySelectorAll('.zf-nums, .zfa-nums')).forEach(one); }
        run(); setTimeout(run,60); setTimeout(run,250); setTimeout(run,600);
        if(document.fonts&&document.fonts.ready){document.fonts.ready.then(function(){run();setTimeout(run,50);});}
        // چھپائی سے عین پہلے ایک بار اور — تاکہ preview میں بالکل ٹھیک بیٹھیں
        window.addEventListener('beforeprint', run);
        try{ var mq=window.matchMedia&&window.matchMedia('print');
          if(mq&&mq.addEventListener) mq.addEventListener('change',function(e){ if(e.matches) run(); }); }catch(e){}
      })();
    </script></body></html>`;
}
