/* ═══════════════════════════════════════════════════════════
   DIGITAL IO — EVIDENCE TAB
   Attach photos, video, audio and documents to cases.
   Live camera capture, file select, print selected.
   ═══════════════════════════════════════════════════════════ */

// ── EVIDENCE ──
registerPage('evidence',renderEvidence);
async function renderEvidence(container){const all=await getEvidence();container.innerHTML=`<div class="page-header"><div><div class="page-title">🔬 Evidence & Media</div><div class="page-subtitle">${all.length} items</div></div><div style="display:flex;gap:8px;"><button class="btn btn-secondary btn-sm" onclick="selectAllEvidence()">☑️ Select All</button><button class="btn btn-secondary btn-sm" onclick="printSelectedEvidence()">🖨️ Print Selected</button><button class="btn btn-primary" onclick="openAddEvidenceModal()">+ Attach Evidence</button></div></div><div class="evidence-grid" id="evidence-grid">${all.length===0?`<div style="grid-column:1/-1;text-align:center;color:var(--text-muted);padding:40px;font-size:12px;">No evidence attached yet.</div>`:all.map(e=>`<div class="evidence-card" id="ev-card-${e.id}"><div class="evidence-thumb" style="position:relative;">${e.file_url?`<img src="${e._viewUrl||e.file_url}" style="width:100%;height:100%;object-fit:cover;" alt="">`:`<span style="font-size:36px;">${e.type==='Photo'?'📷':e.type==='Video'?'🎥':e.type==='Audio'?'🎙️':'📄'}</span>`}<input type="checkbox" class="ev-select-cb" data-id="${e.id}" style="position:absolute;top:8px;left:8px;width:16px;height:16px;accent-color:var(--accent);"></div><div class="evidence-info" onclick="viewEvidenceDetail('${e.id}')"><div class="evidence-name">${esc(e.name)}</div><div class="evidence-tag">${esc(e.fir_number)||'—'}</div><div style="display:flex;justify-content:space-between;margin-top:4px;"><span style="font-size:10px;background:var(--hover-bg);color:var(--text-muted);padding:2px 6px;border-radius:4px;">${e.type}</span><span style="font-size:9px;color:var(--text-faint);">${e.evidence_date||formatDate(e.created_at)}</span></div></div></div>`).join('')}</div>`;}
...
function snapPhoto(){const v=document.getElementById('cam-video'),c=document.getElementById('cam-canvas'),s=document.getElementById('cam-snap');c.width=v.videoWidth;c.height=v.videoHeight;c.getContext('2d').drawImage(v,0,0);s.src=c.toDataURL('image/jpeg',0.9);window._evDataUrl=s.src;window._evFile=null;s.style.display='block';v.style.display='none';stopCamera();const ni=document.getElementById('ev-name');if(ni&&!ni.value)ni.value='Live Photo '+new Date().toLocaleString('en-PK');document.getElementById('ev-type-input').value='Photo';showToast('✅ Photo captured!','success');}
function openFileSelect(){const i=document.createElement('input');i.type='file';i.accept='image/*,video/*,audio/*,.pdf,.doc,.docx';i.onchange=e=>{const f=e.target.files[0];if(!f)return;window._evFile=f;window._evDataUrl=null;const ni=document.getElementById('ev-name');if(ni&&!ni.value)ni.value=f.name;const ti=document.getElementById('ev-type-input');if(ti){if(f.type.startsWith('image'))ti.value='Photo';else if(f.type.startsWith('video'))ti.value='Video';else if(f.type.startsWith('audio'))ti.value='Audio';else ti.value='Document';}showToast('📎 File selected: '+f.name,'success');};i.click();}
async function saveEvidenceItem(){
  ...
  try{
    ...
    // Photo/file (camera dataURL ya select ki hui file) — ab ASAL mein upload/store hoti hai
    const _fileInfo = window._evDataUrl ? { dataUrl: window._evDataUrl }
                    : (window._evFile ? { file: window._evFile } : null);
    const _meta = { name, fir_number:fir, type:evType, evidence_date:evDate, notes:evNotes, is_readonly:true };
    const _row = (typeof addEvidenceWithFile === 'function')
      ? await addEvidenceWithFile(_meta, _fileInfo)
      : await addEvidence(_meta);
    window._evDataUrl = null; window._evFile = null;
