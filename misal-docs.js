...
function getMisalTemplate(docId, c) {
  ...
  // ── Additional proper templates for remaining documents ──
  const moreTemplates = {
    statements_161: `${header('بیان زیر دفعہ 161 ضابطہ فوجداری')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('تھانہ', sta))}
      <div style="margin:10px 0;">
        <div style="font-weight:bold;margin-bottom:4px;">بیان کنندہ کا نام:</div>
        <div style="border:1px solid #ccc;padding:8px;border-radius:4px;" contenteditable="true">نام / ولدیت / سکونت</div>
      </div>
      <div style="font-weight:bold;margin:12px 0 6px;">بیان:</div>
      <div style="min-height:250px;border:1px solid #ccc;padding:12px;border-radius:4px;line-height:2;" contenteditable="true">بیان دیا کہ ...</div>
      <div style="margin-top:10px;font-size:13px;">بیان بحلفہ درست تسلیم کیا۔</div>
      ${sig}`,

    fardat: `${header('فرد')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('تاریخ', dt))}
      <div style="font-weight:bold;margin:10px 0 6px;">بمقام:</div>
      <div style="border:1px solid #ccc;padding:8px;border-radius:4px;margin-bottom:10px;" contenteditable="true">جائے وقوعہ / برآمدگی کا مقام</div>
      <div style="font-weight:bold;margin-bottom:6px;">تحریر فرد:</div>
      <div style="min-height:200px;border:1px solid #ccc;padding:12px;border-radius:4px;line-height:2;" contenteditable="true">آج بتاریخ ... بمقام ... دوران تفتیش ...</div>
      <div style="margin-top:12px;font-weight:bold;">گواہان فرد:</div>
      <div style="border:1px solid #ccc;padding:8px;border-radius:4px;" contenteditable="true">1. ______________  2. ______________</div>
      ${sig}`,

    zamniyat: `${header('ضمنی')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('ضمنی نمبر', '________') + row('تاریخ', dt))}
      <div style="font-weight:bold;margin:10px 0 6px;">تحریر ضمنی:</div>
      <div style="min-height:300px;border:1px solid #ccc;padding:12px;border-radius:4px;line-height:2;" contenteditable="true">آج بتاریخ ... دوران تفتیش مقدمہ ہذا ...</div>
      ${sig}`,

    memorandum: `${header('میمورنڈم برآمدگی')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('تاریخ و وقت', dt))}
      <div style="font-weight:bold;margin:10px 0 6px;">برآمد شدہ اشیاء کی تفصیل:</div>
      <div style="min-height:150px;border:1px solid #ccc;padding:12px;border-radius:4px;line-height:2;" contenteditable="true">1. ...</div>
      <div style="font-weight:bold;margin:12px 0 6px;">بمقام برآمدگی:</div>
      <div style="border:1px solid #ccc;padding:8px;border-radius:4px;" contenteditable="true"></div>
      <div style="font-weight:bold;margin:12px 0 6px;">گواہان:</div>
      <div style="border:1px solid #ccc;padding:8px;border-radius:4px;" contenteditable="true">1. ______________  2. ______________</div>
      ${sig}`,

    arrest_form: `${header('فارم گرفتاری ملزم')}
      ${table(
        row('مقدمہ نمبر', fir) + row('دفعات', sec) +
        row('نام ملزم', '________') + row('ولدیت', '________') +
        row('سکونت', '________') + row('تاریخ گرفتاری', dt) +
        row('وقت گرفتاری', '________') + row('جائے گرفتاری', '________')
      )}
      <div style="font-weight:bold;margin:10px 0 6px;">حلیہ ملزم:</div>
      <div style="min-height:80px;border:1px solid #ccc;padding:10px;border-radius:4px;" contenteditable="true"></div>
      ${sig}`,

    abscond_form: `${header('فارم اشتہاری / مفروری')}
      ${table(
        row('مقدمہ نمبر', fir) + row('دفعات', sec) +
        row('نام ملزم مفرور', '________') + row('ولدیت', '________') +
        row('سکونت', '________')
      )}
      <div style="font-weight:bold;margin:10px 0 6px;">کوائف و حلیہ:</div>
      <div style="min-height:100px;border:1px solid #ccc;padding:10px;border-radius:4px;" contenteditable="true"></div>
      <div style="margin-top:10px;">ملزم موصوف تاحال زیر دفعہ ____ مفرور ہے۔</div>
      ${sig}`,

    warrant: `${header('وارنٹ گرفتاری')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('نام ملزم', '________'))}
      <div style="margin:12px 0;line-height:2;">
        بنام انچارج تھانہ ${sta}،<br>
        حکم دیا جاتا ہے کہ ملزم مذکورہ بالا کو گرفتار کر کے عدالت ہذا میں پیش کیا جائے۔
      </div>
      <div style="margin-top:20px;">تاریخ اجراء: ${dt}</div>
      <div style="margin-top:40px;text-align:left;">
        <div>_______________________</div>
        <div style="font-size:12px;">دستخط و مہر مجاز افسر</div>
      </div>`,

    ishtihar: `${header('اشتہار زیر دفعہ 87 ضابطہ فوجداری')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('نام ملزم', '________'))}
      <div style="margin:12px 0;line-height:2;">
        ہر عام و خاص کو مطلع کیا جاتا ہے کہ ملزم مذکورہ مقدمہ ہذا میں مطلوب ہے۔
        ملزم کو حکم دیا جاتا ہے کہ بتاریخ ______ عدالت میں حاضر ہو، بصورت دیگر قانونی کاروائی عمل میں لائی جائے گی۔
      </div>
      ${sig}`,

    progress: `${header('پراگریس رپورٹ تفتیش')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('تاریخ رپورٹ', dt))}
      <div style="font-weight:bold;margin:10px 0 6px;">تفتیش کی پیش رفت:</div>
      <div style="min-height:250px;border:1px solid #ccc;padding:12px;border-radius:4px;line-height:2;" contenteditable="true">اب تک کی تفتیش کے مطابق ...</div>
      ${sig}`,

    inkishafat: `${header('انکشافات ملزم')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('نام ملزم', '________'))}
      <div style="font-weight:bold;margin:10px 0 6px;">انکشافات:</div>
      <div style="min-height:250px;border:1px solid #ccc;padding:12px;border-radius:4px;line-height:2;" contenteditable="true">ملزم نے دوران تفتیش انکشاف کیا کہ ...</div>
      ${sig}`,

    staff: `${header('ہمراہی ملازمان')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('تاریخ', dt))}
      <table style="width:100%;border-collapse:collapse;margin-top:10px;">
        <thead><tr>
          <th style="border:1px solid #ccc;padding:6px;">نمبر شمار</th>
          <th style="border:1px solid #ccc;padding:6px;">نام</th>
          <th style="border:1px solid #ccc;padding:6px;">عہدہ</th>
          <th style="border:1px solid #ccc;padding:6px;">بلٹ نمبر</th>
        </tr></thead>
        <tbody>
          ${[1,2,3,4,5].map(i=>`<tr>
            <td style="border:1px solid #ccc;padding:8px;text-align:center;">${i}</td>
            <td style="border:1px solid #ccc;padding:8px;" contenteditable="true"></td>
            <td style="border:1px solid #ccc;padding:8px;" contenteditable="true"></td>
            <td style="border:1px solid #ccc;padding:8px;" contenteditable="true"></td>
          </tr>`).join('')}
        </tbody>
    ...
    darkhwastain: `${header('درخواست')}
      ${table(row('مقدمہ نمبر', fir) + row('دفعات', sec) + row('تاریخ', dt))}
      <div style="margin:10px 0;">بخدمت جناب ____________</div>
      <div style="font-weight:bold;margin:10px 0 6px;">موضوع:</div>
      <div style="border:1px solid #ccc;padding:8px;border-radius:4px;margin-bottom:10px;" contenteditable="true"></div>
      <div style="font-weight:bold;margin-bottom:6px;">تحریر درخواست:</div>
      <div style="min-height:200px;border:1px solid #ccc;padding:12px;border-radius:4px;line-height:2;" contenteditable="true">جناب عالی، گزارش ہے کہ ...</div>
      ${sig}`,

    brief: `${header('بریف مقدمہ')}
      ${table(
        row('مقدمہ نمبر', fir) + row('دفعات', sec) +
        row('مدعی', cmp) + row('تاریخ وقوعہ', c?.occurrence_date||'________')
      )}
      <div style="font-weight:bold;margin:10px 0 6px;">خلاصہ مقدمہ:</div>
      <div style="min-height:200px;border:1px solid #ccc;padding:12px;border-radius:4px;line-height:2;" contenteditable="true">مختصر حالات واقعہ ...</div>
      ${sig}`,
  };
  Object.assign(templates, moreTemplates);
