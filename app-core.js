...
const pageTitles={dashboard:'🏠 Dashboard',cases:'📁 My Cases',misal:'📄 MISAL Builder',forms:'📥 Official Forms',fivec:'📋 5-C Applications',evidence:'🔬 Evidence',search:'🔍 Advanced Search',law:'⚖️ Law Library',reminders:'🔔 Reminders',performance:'📊 Performance',backup:'☁️ Backup & Sync',settings:'⚙️ Settings',admin:'👨‍💼 Admin Panel'};
const pageRenderers={};
...
window.addEventListener('DOMContentLoaded',async()=>{onSupabaseReady(async()=>{await checkExistingSession();});setInterval(()=>{const el=document.getElementById('footer-time');if(el)el.textContent=new Date().toLocaleTimeString('en-PK');},1000);console.log('✅ Digital IO v4.1.0 — MODULAR (Round 1: core+dashboard+cases) — '+new Date().toISOString());});
